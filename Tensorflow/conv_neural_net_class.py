...
n_classes = 1380
hm_inbatch = 64
hm_epochs = 1000 #cycles = feed forward + backprop
...
val_filename_queue = tf.train.string_input_producer([validation_tfrecords_filename])# add num_epochs to limit amount of time through data
...
def test_neural_network():
    ...
    with tf.Session() as sess:
        ...
        accuracy_sum = tf.cast(correct,'float')
...
def batch_test_neural_network():
    ...
    with tf.Session() as sess:
        ...
        good = 0
        for epoch in range(int(num_val_samples/hm_inbatch)):
            ...
            #val_imgs = val_imgs.reshape(hm_inbatch,76800)
            val_imgs = val_imgs.reshape(hm_inbatch,4800) # resize

            #convert labels to sparse one hot 
            targets = np.array(val_lbls).reshape(-1)
            val_lbls = np.eye(n_classes)[targets]

            good += accuracy_sum.eval(feed_dict={x:val_imgs,y:val_lbls})
            print('Epoch ' + str(epoch) + ' of ' + str(int(num_val_samples/hm_inbatch)))

    print(good)

    print('Batch Test Accuracy: %g'%(good/(num_val_samples-leftover)))
